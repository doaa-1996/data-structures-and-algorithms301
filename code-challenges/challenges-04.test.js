...
const citiesAtoJ = (arr) => {
  ...
  arr.forEach((element,idx) => {
    let result = element.match(rejex);
    if(result)
...
const matchMonth = (input) => {
  let rejex1 = /\b(O)ctober/g, rejex2 = /\b(O)c(t)\b/, rejex3 = /\b(o)ctober/, rejex4 = /\b(o)c(t)\b/;
  return rejex1.test(input) || rejex2.test(input) || rejex3.test(input) || rejex4.test(input)
};
