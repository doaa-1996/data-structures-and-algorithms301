...
// Solution code here...
  ...
  let length = arr.map((str, index) => [str.length, index]);
  let longestStr = length.reduce((longestStr, currentStr ) => {
    return currentStr[0] > longestStr[0] ? currentStr : longestStr;
  },length[0]);
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(phoneNumber => phoneNumber.substr(1, 3) + phoneNumber.substr(6, 3) + phoneNumber.substr(10, 4));
...
const allHappy = (arr) => {
  // Solution code here...
  return arr.reduce((isAllHappy, str) => isAllHappy && str.includes(':)'), true);
};
